/* ============================================================
   Oson Studio — AI Agentlar kursi sertifikati chizish (umumiy modul)
   window.AgentCert.draw(canvas, { name, dateISO })
   Smaragd (teal) temada; muhr o'rtasida ROBOT (agent) emblemasi.
...
(function (global) {
  ...
  // Robot (agent) emblemasi
  function drawRobot(x, cx, cy, col) {
    x.strokeStyle = col; x.fillStyle = col; x.lineWidth = 2.3; x.lineCap = "round"; x.lineJoin = "round";
    // antena
    x.beginPath(); x.moveTo(cx, cy - 15); x.lineTo(cx, cy - 10); x.stroke();
    x.beginPath(); x.arc(cx, cy - 16.5, 2, 0, 7); x.fill();
    // bosh
    x.lineWidth = 2.2; roundRect(x, cx - 12, cy - 10, 24, 21, 5); x.stroke();
    // ko'zlar
    x.beginPath(); x.arc(cx - 5, cy - 1, 2.4, 0, 7); x.fill();
    x.beginPath(); x.arc(cx + 5, cy - 1, 2.4, 0, 7); x.fill();
    // og'iz
    x.lineWidth = 2; x.beginPath(); x.moveTo(cx - 5, cy + 5.5); x.lineTo(cx + 5, cy + 5.5); x.stroke();
    // quloqlar
    x.lineWidth = 2.2; x.beginPath(); x.moveTo(cx - 12, cy - 2); x.lineTo(cx - 15, cy - 2); x.moveTo(cx + 12, cy - 2); x.lineTo(cx + 15, cy - 2); x.stroke();
  }
  function drawSeal(x, cx, cy, gold, accent, accent2) {
    ...
    drawRobot(x, cx, cy, "#fffef9");
  }
  function makeQR(text, size) {
    if (!global.QRCode) return null;
    try {
      var div = document.createElement("div");
      new global.QRCode(div, { text: text, width: size, height: size, colorDark: "#0f766e", colorLight: "#ffffff", correctLevel: global.QRCode.CorrectLevel.M });
  ...
  function certURL(o) {
    ...
    return SITE + "/agent-sertifikat.html?" + p.toString();
  }
  ...
  function draw(canvas, o) {
    ...
    var accent = "#0d9488", accent2 = "#0f766e", gold = "#c79a3a", ink = "#1f2937", soft = "#6b7280";

    x.fillStyle = "#fffdf7"; x.fillRect(0, 0, W, H);
    var rg = x.createRadialGradient(W / 2, -40, 40, W / 2, -40, 780);
    rg.addColorStop(0, "rgba(13,148,136,.10)"); rg.addColorStop(1, "rgba(255,255,255,0)");
    ...
    x.fillStyle = soft; x.font = "600 15px system-ui, sans-serif"; x.fillText("AI Agentlar — noldan amaliyotgacha (Claude bilan)", W / 2, 180);
    ...
    wrapCenter(x, "egasiga «AI Agentlar» onlayn kursini to'liq va muvaffaqiyatli yakunlaganligi uchun beriladi.", W / 2, 348, W - 280, 24);
  ...
  global.AgentCert = { draw: draw, makeQR: makeQR, certURL: certURL, fmtDate: fmtDate, UZ_MONTHS: UZ_MONTHS };
})(window);
