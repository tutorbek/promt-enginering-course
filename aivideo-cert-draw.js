/* ============================================================
   Oson Studio — "AI bilan video yaratish" kursi sertifikati (umumiy modul)
   window.AiVideoCert.draw(canvas, { name, dateISO })
   Kinematografik qizil/to'q sariq tema; muhr o'rtasida PLAY uchburchagi.
...
(function (global) {
  ...
  // PLAY uchburchagi — video belgisi
  function drawPlay(x, cx, cy, col) {
    x.fillStyle = col;
    x.beginPath();
    x.moveTo(cx - 7, cy - 11);
    x.lineTo(cx + 12, cy);
  ...
  function drawSeal(x, cx, cy, gold, accent, accent2) {
    ...
    drawPlay(x, cx + 1, cy, "#fffef9");
  }
  function makeQR(text, size) {
    if (!global.QRCode) return null;
    try {
      var div = document.createElement("div");
      new global.QRCode(div, { text: text, width: size, height: size, colorDark: "#c2410c", colorLight: "#ffffff", correctLevel: global.QRCode.CorrectLevel.M });
  ...
  function certURL(o) {
    ...
    return SITE + "/aivideo-sertifikat.html?" + p.toString();
  }
  ...
  function draw(canvas, o) {
    ...
    var accent = "#dc2626", accent2 = "#ea580c", gold = "#c79a3a", ink = "#1f2937", soft = "#6b7280";

    x.fillStyle = "#fffdf7"; x.fillRect(0, 0, W, H);
    var rg = x.createRadialGradient(W / 2, -40, 40, W / 2, -40, 780);
    rg.addColorStop(0, "rgba(220,38,38,.10)"); rg.addColorStop(1, "rgba(255,255,255,0)");
    ...
    x.fillStyle = soft; x.font = "600 15px system-ui, sans-serif"; x.fillText("AI bilan video yaratish", W / 2, 180);
    ...
    wrapCenter(x, "egasiga «AI bilan video yaratish» onlayn kursini noldan advanced darajagacha to'liq va muvaffaqiyatli yakunlaganligi uchun beriladi.", W / 2, 348, W - 280, 24);
  ...
  global.AiVideoCert = { draw: draw, makeQR: makeQR, certURL: certURL, fmtDate: fmtDate, UZ_MONTHS: UZ_MONTHS };
})(window);
